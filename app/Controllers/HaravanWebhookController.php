...
class HaravanWebhookController
{
    ...
    //     // Haravan gửi org_id = slug của shop → đổi sang internal UUID
    //     $orgId = $this->resolveOrgId($haravanOrgId);
    //     if (!$orgId) {
    //         $this->writeLog($topic, $haravanOrgId, 'error', 'Organization not found: ' . json_encode($payload));
    //         exit;
    //     }
        ...
        try {
            switch ($topic) {
                ...
                case 'orders/cancelled':
                    ...
                    break;
                // BỔ SUNG CÁC CASE NÀY:
                case 'customers/create':
                case 'customers/update':
                    $this->handleCustomerUpsert($payload, $orgId);
                    break;
                case 'products/update':
                case 'products/delete': // Nếu muốn xóa service khi Haravan xóa sp
                    $this->handleProductUpdate($payload, $orgId);
                    break;
                case 'app/uninstalled':
                    $this->handleAppUninstalled($orgId);
